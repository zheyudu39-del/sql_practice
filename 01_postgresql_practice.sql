-- ============================================================
-- PostgreSQL 18 练习库：电商订单 (shop)
-- 运行方式（会提示输入 postgres 密码）：
--   "D:\PostgreSQL\bin\psql.exe" -U postgres -f 01_postgresql_practice.sql
-- 或进入交互后：  \i 01_postgresql_practice.sql
-- ============================================================
...
-- ============================================================
-- E. 窗口函数（PostgreSQL 强项，MariaDB 10.2+ 也支持）
-- ============================================================
...
SELECT UPPER('hello') AS 大写, LOWER('HELLO') AS 小写,
       LENGTH('数据库') AS 长度,
       SUBSTRING('PostgreSQL' FROM 1 FOR 4) AS 截取,
       REPLACE('a-b-c','-','/') AS 替换;
...
SELECT CURRENT_DATE AS 今天,
       CURRENT_TIMESTAMP AS 此刻,
       AGE(DATE '2026-09-06', DATE '2025-01-10') AS 间隔,
       TO_CHAR(CURRENT_DATE, 'YYYY年MM月DD日') AS 格式化,
       CURRENT_DATE + INTERVAL '7 days' AS 一周后;
...
-- H6 字符串聚合 STRING_AGG（PostgreSQL 特色）
SELECT city, STRING_AGG(name, '、' ORDER BY name) AS 客户名单
FROM customers GROUP BY city;

-- H7 类型转换
SELECT '123'::INT + 1 AS 转整数,
       price::TEXT AS 转文本 FROM products LIMIT 3;
